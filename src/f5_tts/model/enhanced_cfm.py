...
class EnhancedCFM(nn.Module):
    ...
    def __init__(
        ...
        audio_drop_exclusive_uncond: bool = False,
        num_channels=None,
        ...
        duration_use_speaker: bool = True,
        ...
        self.audio_drop_exclusive_uncond = audio_drop_exclusive_uncond
        ...
        if use_duration_predictor:
            ...
            self.duration_predictor = DurationPredictor(
                ...
                use_speaker=duration_use_speaker,
            )
    ...
    def sample_condition_dropout(self, batch: int, device):
        ...
        # NOTE on audio dropout statistics: drop_audio mixes TWO schemes —
        #   (1) an independent Bernoulli(audio_drop_prob) drawn here for every
        #       sample, and
        #   (2) a categorical force-to-True inside the `uncond` mode below.
        # They compose, so the TRUE marginal is NOT audio_drop_prob but:
        #     P(drop_audio) = audio_drop_prob + (1 - audio_drop_prob) * P(uncond)
        #   e.g. 0.3 + 0.7 * 0.2 = 0.44, not 0.30.
        # Consequently every NON-uncond mode (full, voice_only, no_emotion, …) is
        # ALSO audio-dropped ~audio_drop_prob of the time — the modes are not
        # "audio-clean". This is intentional IF you want the model to handle both
        # prompted and prompt-free generation orthogonally to the expression modes.
        #
        # When `audio_drop_exclusive_uncond` is set, we correct the double-count so
        # the overall marginal equals audio_drop_prob exactly: uncond always drops
        # audio (it must), and the independent Bernoulli is applied to NON-uncond
        # rows at the reduced rate b = (p − p_unc)/(1 − p_unc) so that
        #   p_unc·1 + (1−p_unc)·b = p   (the declared audio_drop_prob).
        if self.audio_drop_exclusive_uncond:
            is_uncond = torch.tensor(
                [m == "uncond" for m in mode_names], device=device, dtype=torch.bool)
            p = self.audio_drop_prob
            p_unc = float(self.dropout_mode_probs.get("uncond", 0.0))
            b = 0.0 if p_unc >= 1.0 else max(0.0, (p - p_unc) / (1.0 - p_unc))
            rand = torch.rand(batch, device=device)
            drop_audio = is_uncond | (~is_uncond & (rand < b))
        else:
    ...
    def sample(
        ...
        # Learnable timbre at inference: compute the global timbre vector from the
        # reference mel (`cond` is the prompt mel here, cond_mask marks valid frames)
        # and attach it, mirroring training so the speaker path is augmented the same
        # way. Uses the original (pre-pad) cond region via cond_mask.
        timbre_enc = getattr(self.transformer, "timbre_encoder", None)
        if timbre_enc is not None and model_conditions is not None:
            ref_mask = cond_mask if 'cond_mask' in dir() else None
            try:
                g_timbre, _ = timbre_enc(cond, mask=ref_mask)
                model_conditions.timbre_raw = g_timbre
        ...
        # Initial noise
        # Seed ONCE before the loop, not inside it. Re-seeding every iteration would
        # reset the RNG to the same state for each sample → identical noise for the
        # whole batch (kills diversity, and for batched generation makes every output
        # the same). Seeding once keeps reproducibility while giving each sample its
        # own draw.
        if exists(seed):
    ...
    def forward(
        ...
        # Learnable timbre (multi-level): if the transformer has a timbre encoder,
        # extract a global timbre vector from the PROMPT mel (the non-target region)
        # and attach it to the conditions so project_speaker can augment the WavLM-SV
        # path. Using the prompt region (~rand_span_mask) — the same frames kept in
        # `cond` — means the timbre reference and the generated target are disjoint
        # frames, so there is NO leakage of the target into the timbre signal.
        timbre_enc = getattr(self.transformer, "timbre_encoder", None)
        if timbre_enc is not None and model_conditions is not None:
            prompt_mask = (~rand_span_mask) & mask if exists(mask) else (~rand_span_mask)
            # x1 is the clean mel; prompt frames are where prompt_mask is True.
            g_timbre, _ = timbre_enc(x1, mask=prompt_mask)
            try:
                model_conditions.timbre_raw = g_timbre
            except (AttributeError, TypeError):
                pass
        ...
        # Cross-orthogonal disentanglement loss (Marco-Voice / Trans-Ort style).
        # Forces speaker and emotion embeddings to be orthogonal (independent) at
        # the batch level → reduces speaker leakage into emotion (and vice-versa),
        # which is the residual entanglement that hurts cross-speaker / cross-lingual
        # transfer. NO emotion labels needed — purely geometric.
        ortho_loss = torch.tensor(0.0, device=device)
        if getattr(self, "ortho_loss_weight", 0.0) > 0 and model_conditions is not None:
            s = getattr(model_conditions, "speaker", None)
            e = getattr(model_conditions, "emotion_global", None)
            if s is not None and e is not None and s.shape[0] > 1:
                # L2-normalize per sample, then penalize cross dot-products.
                s_n = F.normalize(s.float(), dim=-1)          # (B, Ds)
                e_n = F.normalize(e.float(), dim=-1)          # (B, De)
                # Only the matched pairs (same utterance) need to be orthogonal;
                # ||diag(E Sᵀ)||² = Σ_i (eᵢ·sᵢ)². Dims may differ → project to min.
                d = min(s_n.shape[-1], e_n.shape[-1])
                pair_dot = (s_n[:, :d] * e_n[:, :d]).sum(-1)  # (B,) cosine per pair
                ortho_loss = (pair_dot ** 2).mean()
        ...
        if masked.numel() == 0:
            return (pred * 0.0).sum(), cond, pred, mode_names, dur_loss, ortho_loss
        return masked.mean(), cond, pred, mode_names, dur_loss, ortho_loss
